...
export function vouche(amount,proposalId,callback) {
    return dispatch => {
        //dispatch(request({ userId }));

        service.vouche(amount,proposalId)
            .then(
                data => {
                  console.log("vouch result",data)
                    dispatch(success(data));
                    callback()
                    //  history.push('/');
                },
...
export function fake(amount,proposalId, callback) {
    return dispatch => {
        //dispatch(request({ userId }));

        service.fake(amount,proposalId)
            .then(
                data => {
                    dispatch(success(data));
                    callback();
